	...
	private final EditProductButton createEditProductButton() {
		return new EditProductButton() {
			...
			public Product[] getSelectedProduct() {
				Product main_product, sub_products[], products[];
				
				main_product = table.getSelectedProduct();
				if(main_product.getPackaging().getParentPackId() == -1) {
					sub_products = table.getSelectedProductChildrenPrime();
					products = new Product[1 + sub_products.length];
					
					products[0] = main_product;
					for(int i=0; i<sub_products.length; i++) {
						products[i+1] = sub_products[i];
					}
