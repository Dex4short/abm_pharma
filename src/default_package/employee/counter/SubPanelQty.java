...
public abstract class SubPanelQty extends ActionPanel implements UnitConverter{
	...
	@Override
	public void onOk() {
		Packaging packaging, newPackaging, bypackagings[];
		...
		if(qty2 <= 0) {
			...
			ABM_Pharma.getWindow().getDisplayPanel().floatMessage("invalid input.");
			return;
		}
		...
		newPackaging = new Packaging(-1, qty, uom2, parentPack_id);
		...
		onProductQtyOk(newPackaging, bypackagings);
	}
	@Override
	...
	public abstract void onProductQtyOk(Packaging newPackaging,Packaging bypackagings[]);
